...
fn select_neighbors(layer: &HNSWLayer, vec: &Vector, max_n: Int) -> Vec[UInt64] {
  ...
  var pairs = Vec[SearchResult].new();
  var i: Int = 0;
  while i < layer.nodes.len() {
    var dist = vector_distance(&layer.vectors[i], vec, Euclidean);
    pairs.push(SearchResult{ id: layer.nodes[i].id; distance: dist; });
    i = i + 1;
  }
  var sorted: Int = 0;
...
pub fn hnsw_search(graph: &HNSWGraph, query: &Vector, k: Int) -> Vec[SearchResult]
  requires: k > 0
{
  ...
  var candidates = Vec[SearchResult].new();
  var cur_dist = vector_distance(&graph.layers[0].vectors[current], query, Euclidean);
  candidates.push(SearchResult{ id: graph.layers[0].nodes[current].id; distance: cur_dist; });
  var frontier: Int = 0;
      ...
      while j < node.neighbors.len() {
        ...
        if !already {
          ...
          if npos != -1 {
            ...
            candidates.push(SearchResult{ id: nid; distance: ndist; });
          }
        }
        j = j + 1;
      }
