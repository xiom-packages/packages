...
{
  let n = bindings.len();
  let h = unsafe { xvk_desc_set_layout_create(app, bindings as *Int32, (n / 4) as Int32) };
  if h == 0 {
  ...
  requires: app != 0
{
  let h = unsafe {
    xvk_pipeline_create_graphics(
      ...
      (if depth_test { 1 } else { 0 }) as Int32,
      (if depth_write { 1 } else { 0 }) as Int32,
      (if blend { 1 } else { 0 }) as Int32,
      vert_shader, frag_shader, layout, render_pass,
      bindings as *Int32, (bindings.len() / 3) as Int32,
      attributes as *Int32, (attributes.len() / 4) as Int32
    )
  };
  if h == 0 {
...
pub fn desc_pool_create(app: Int, pool_sizes: Vec[Int32], max_sets: Int) -> Result[Int, Str]
  requires: app != 0
{
  let h = unsafe { xvk_desc_pool_create(app, pool_sizes as *Int32, (pool_sizes.len() / 2) as Int32, max_sets as Int32) };
  if h == 0 {
...
pub fn render_pass_create(app: Int, color_formats: Vec[Int32], depth_format: Int) -> Result[Int, Str]
  requires: app != 0
{
  let h = unsafe { xvk_render_pass_create(app, color_formats as *Int32, (color_formats.len() / 4) as Int32, depth_format as Int32) };
  if h == 0 {
