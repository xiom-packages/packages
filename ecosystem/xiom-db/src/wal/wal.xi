module xiom.db.wal.wal
use xiom.db.wal.wal_record;
use xiom.db.index.btree;

// Write-ahead log: the ordered sequence of `WALEntry` records that makes the
// engine crash-recoverable. Mutations are appended here BEFORE they touch the
// index (WAL-before-data), so `wal_replay` can rebuild the B-tree from the log
// after a restart. This phase keeps the log in memory; Phase 2 backs it with a
// durable, fsync'd segment file via xiom-core's wal_writer.
...
// Replay every record onto `target` in log order. Returns false if any single
// entry failed to apply (e.g. update/delete of a missing key), but keeps going
// so partial recovery still restores as much state as possible.
...
// Drop every record older than `before_timestamp` (a checkpoint boundary).
