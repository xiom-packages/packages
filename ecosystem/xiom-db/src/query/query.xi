module xiom.db.query.query
use xiom.db.index.btree;

// Declarative query construction and execution against a B-tree index. A
// `Query` is a conjunction (AND) of comparison conditions with optional
// pagination. The current execution strategy is a full in-order scan of the
// tree's values followed by predicate filtering; the index-aware planner that
// pushes range predicates down into `btree_range_query` lands in Phase 3
// (see query/planner.xi).
...
// Execute the query against `tree`. Deterministic: values are visited in the
// B-tree's in-order sequence, filtered by every condition (AND), then offset is
// skipped and limit truncates the tail.
