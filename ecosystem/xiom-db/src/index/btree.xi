...
// Canonical in-memory B-tree, flat-array representation. XIOM's borrow checker
// forbids storing references inside structs, so all nodes live in a single
// `Vec[BTreeNode]` and parent/child links are integer indices into that vec.
// This is the WORKING index implementation for the whole engine; every mutating
// path (insert/delete) preserves the ordering and balance invariants documented
// in docs/contracts-and-invariants.md.
