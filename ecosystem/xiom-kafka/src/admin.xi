...
fn kafka_create_topic(brokers: Str, topic: Str, partitions: Int, replication: Int) -> Result[Unit, KafkaError] {
  let _ = brokers;
  let _ = topic;
  let _ = partitions;
  let _ = replication;
...
fn kafka_delete_topic(brokers: Str, topic: Str) -> Result[Unit, KafkaError] {
  let _ = brokers;
  let _ = topic;
  Err(KafkaError {
    code: -999,
