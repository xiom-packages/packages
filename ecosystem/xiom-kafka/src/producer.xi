...
fn kafka_produce(producer: &KafkaProducer, topic: Str, key: &Vec[Int], value: &Vec[Int]) -> Result[(), KafkaError] {
  let _ = producer;
  let _ = key;
  if topic.len() == 0 {
...
fn kafka_producer_flush(producer: &KafkaProducer, timeout_ms: Int) -> Result[(), KafkaError] {
  let _ = producer;
  let _ = timeout_ms;
  Ok(())
}
