...
> ## [OK] ALL HISTORICAL GAPS CLOSED (v0.33.0 through v0.48.0)
...
> |----|---------|--------|------------|
> | CG-01 | Float32/Float64 Vec element reads return garbage | [WARN] Reproduced | Use scalar Float32 FFI only. Float arrays must route through C-side staging |
> | CG-02 | E001 "use of moved value" on Float64 in math.sin() calls | [WARN] Cosmetic | Use separate `now()` bindings for each call |
> | CG-03 | Multi-module catalog doesn't resolve `use xiom.vulkan` from single file | [WARN] Works via merge | Pass all .xi files on xiom command line |
> 
...
> |-----|---------|----------------|
> | GAP-1  | qualified `Type.Variant` in match | [OK] CLOSED (regress_gap1) |
> | GAP-2  | `extern "C" { }` blocks | [OK] CLOSED (regress_gap2) |
> | GAP-3  | `const` / `pub const` refs | [OK] CLOSED (check test_gap3_*) |
> | GAP-4  | `=>` implication in contracts | [OK] CLOSED |
> | GAP-5  | `\0 \b \u{}` escapes | [OK] CLOSED (regress_gap5) |
> | GAP-6  | `_` in user enum payload | [OK] CLOSED (regress_gap6) |
> | GAP-8  | bitwise/shift `^ & \| ~ << >>` | [OK] CLOSED (regress_gap8) |
> | GAP-9  | `type X = enum {...}` | [OK] CLOSED (regress_gap9) |
> | GAP-10 | trailing `;` after control block | [OK] CLOSED (regress_gap10) |
> | GAP-11 | tail expression (implicit return) | [OK] CLOSED (regress_gap11) |
> | GAP-12 | unit literal `()` / `Ok(())` | [OK] CLOSED (regress_gap12) |
> | GAP-13 | brace module `module x { }` | [OK] CLOSED -- parser supports both forms (e2e test) |
> | GAP-14 | bare `is Ok` / `is Err` | [OK] CLOSED |
> | cross-module `use` | multi-file build | [OK] RESOLVED -- stdlib search path + transitive extern/const injection landed; `use xiom.*` resolves.
>
...
>
> Note: XIOM's core guarantee is "if it compiles, it's safe". [OK] **RESOLVED (v0.47.8):** the
...
- **Status: [OK] CLOSED (2026-07-11)** -- The parser already handles brace-form modules correctly. Verified with e2e test `examples/e2e/brace_module.xi`. Both `module x { ... }` (brace-form) and `module x;` (file-form) work.
- **Spec conflict:** formal grammar (`specs/XIOM_Language_Spec.md` S3.2) shows brace form; AI_CONTEXT.md L473 shows file-form. Both are valid -- the parser accepts both.
...
`<source.xi>` entry; AI_CONTEXT.md Smulti-file (L2404-2503) describes dotted `module`/`use`
resolution.
...
| GAP-13 | brace module | P001 | spec S3.2 vs L473 | 1 |
