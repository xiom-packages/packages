...
REST helpers are valuable, but they are not the same thing as the HTTP transport. Keeping them separate lets `xiom-http` stay minimal while `xiom-rest` evolves conventions, response shapes, and docs generation without forcing those opinions into every HTTP user. [grizzlypeaksoftware](https://www.grizzlypeaksoftware.com/library/expressjs-router-patterns-for-large-applications-zz0h79so)
...
### `src/pagination.xi`
Cursor and page-number pagination primitives. REST guidance strongly recommends explicit pagination to keep large result sets predictable and stable. [zuplo](https://zuplo.com/learning-center/api-design-patterns)
...
|---|---|
| Resource-oriented URI design | `resource<T>()` builders with collection/item subroutes  [zuplo](https://zuplo.com/learning-center/api-design-patterns) |
| Nested routers | Route composition through module-scoped builders  [grizzlypeaksoftware](https://www.grizzlypeaksoftware.com/library/expressjs-router-patterns-for-large-applications-zz0h79so) |
| Pagination | Typed `PageRequest` / `CursorPage` models  [zuplo](https://zuplo.com/learning-center/api-design-patterns) |
| Filtering/sorting | Structured query types with contract validation  [zuplo](https://zuplo.com/learning-center/api-design-patterns) |
| Content negotiation | Explicit `Accept` handling with typed serializers  [strapi](https://strapi.io/blog/restful-api-design-guide-principles-best-practices) |
| Versioning | Versioned route groups and deprecation metadata  [zuplo](https://zuplo.com/learning-center/api-design-patterns) |
| Error handling | Typed error-to-status mapping with stable payload shape  [deepdocs](https://deepdocs.dev/rest-api-best-practices/) |
