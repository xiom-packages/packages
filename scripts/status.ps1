...
#   .\scripts\status.ps1 -Action report         # regenerate docs/PACKAGE_STATUS.md
...
[CmdletBinding()]
param(
    [ValidateSet("list", "validate", "seed", "repin", "report", "update")]
    [string]$Action = "list",
    [string]$Package = "",
    [string]$Stage = "",
    [string]$TestsStatus = "",
...
switch ($Action) {
    ...
    "report" {
        $outPath = if ($OutFile) { $OutFile } else { Join-Path $repoRoot "docs\PACKAGE_STATUS.md" }
        ...
        foreach ($dir in (Get-ImplementedPackages)) {
            ...
            if (Test-Path -LiteralPath $statusPath) {
                ...
                $rows += [pscustomobject]@{
                    ...
                    Folder  = $dir.Name
                    Stage   = $s.stage
                    Suite   = [string]$s.tests.suite
                    Tests   = $tests
                    Commit  = [string]$s.tests.commit
                    Publish = [bool]$s.publish
                    Notes   = [string]$s.excluded_reason
                }
            } else {
                $rows += [pscustomobject]@{
                    ...
                    Commit  = ""
                    Publish = $false
                    Notes   = "run -Action seed"
                }
            }
        }

        $stageOrder = @{ "stable" = 0; "ported" = 1; "incubating" = 2; "deprecated" = 3 }
        $sorted = @($rows | Sort-Object @{ Expression = { if ($stageOrder.ContainsKey($_.Stage)) { $stageOrder[$_.Stage] } else { 9 } } }, Package)
        $stageCounts = @($rows | Group-Object Stage | Sort-Object Name)
        $now = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")

        $sb = New-Object System.Text.StringBuilder
        [void]$sb.AppendLine("# XIOM Packages -- Status and Publish Green-Light")
        [void]$sb.AppendLine("")
        [void]$sb.AppendLine("<!-- Generated by scripts/status.ps1 -Action report. Do not edit by hand. -->")
        [void]$sb.AppendLine("<!-- Copyright (c) 2026 Eleftherios Notas and The XIOM Authors -->")
        [void]$sb.AppendLine("<!-- SPDX-License-Identifier: MIT OR Apache-2.0 -->")
        [void]$sb.AppendLine("")
        [void]$sb.AppendLine("Toolchain pin: ``$pin`` -- generated $now.")
        [void]$sb.AppendLine("")
        [void]$sb.AppendLine("## Summary")
        [void]$sb.AppendLine("")
        [void]$sb.AppendLine("- packages tracked: $($rows.Count)")
        foreach ($g in $stageCounts) {
            [void]$sb.AppendLine("- $($g.Name): $($g.Count)")
        }
        $publishCount = @($rows | Where-Object { $_.Publish }).Count
        [void]$sb.AppendLine("- publish enabled: $publishCount")
        [void]$sb.AppendLine("")
        [void]$sb.AppendLine("## Publish green-light checklist (per package)")
        [void]$sb.AppendLine("")
        [void]$sb.AppendLine("1. ``stage: stable`` -- conformance suite green on the pinned compiler, recorded with ``run_by``, ``commit``, ``passed``/``failed`` (``scripts/port.ps1`` + ``scripts/status.ps1 -Action update``).")
        [void]$sb.AppendLine("2. Package name listed in ``.github/publish-allowlist.txt``.")
        [void]$sb.AppendLine("3. Registry scope enumerated for the dotted name (registry session).")
        [void]$sb.AppendLine("4. CI gates ready: ``registry-publish`` environment (repo protection decision) and the ``XIOM_SIGNING_KEY`` secret for a stable first-party signature.")
        [void]$sb.AppendLine("5. First publish is a canary (``workflow_dispatch`` with ``package=<name>``, staging registry) followed by an install-back check.")
        [void]$sb.AppendLine("6. Production enablement only after the staging canary verifies.")
        [void]$sb.AppendLine("")
        [void]$sb.AppendLine("Versions are immutable forever; a published version can never be replaced.")
        [void]$sb.AppendLine("")
        [void]$sb.AppendLine("## Packages")
        [void]$sb.AppendLine("")
        [void]$sb.AppendLine("| Package | Folder | Stage | Suite | Tests | Run commit | Publish | Notes |")
        [void]$sb.AppendLine("|---|---|---|---|---|---|---|---|")
        foreach ($r in $sorted) {
            $notes = if ($r.Notes) { $r.Notes.Replace("|", "/") } else { "" }
            $suite = if ($r.Suite) { $r.Suite } else { "" }
            [void]$sb.AppendLine("| $($r.Package) | $($r.Folder) | $($r.Stage) | $suite | $($r.Tests) | $($r.Commit) | $($r.Publish) | $notes |")
        }
        [void]$sb.AppendLine("")

        [System.IO.File]::WriteAllText($outPath, $sb.ToString(), (New-Object System.Text.UTF8Encoding($false)))
        Write-Host "status report: $outPath ($($rows.Count) packages)"
        exit 0
    }
