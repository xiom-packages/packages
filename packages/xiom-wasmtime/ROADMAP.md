...
## Current Rating: 7/10 ⚙️ SPEC-READY (FFI stub)

| Criterion | Status |
|-----------|--------|
| ✅ Extern "C" declarations | 11 FFI functions declared: Engine, Store, Module, Instance, Func, Functype, Valtype |
| ✅ Safe wrappers | 11 pub fn: engine_new/delete, store_new/delete, module_new/delete, instance_new/delete, func_call, functype_new, valtype_new |
| ✅ Design-by-contract | 13 contracts across 11 functions (+1 ensures) |
| ✅ Tests | test_conformance.xi — 46 tests, 12 sections |
| ✅ SPEC.md | Full API surface documented |
| ✅ ROADMAP.md | This file |
| ⚠️ C bridge linking | Requires system-installed Wasmtime C API at link time |
...
|-------|--------|-------------|
| **P1: Core FFI** | ✅ Done | extern "C" declarations for wasmtime engine, store, module, instance, func |
| **P1: Safe Wrappers** | ✅ Done | 11 safe wrappers with design-by-contract (requires/ensures) |
| **P1: Tests** | ✅ Done | 46 conformance tests (types, constants, stubs, contracts, error handling, lifecycle, API presence) |

## API Surface

| Function | Signature | Contracts | Status |
|----------|-----------|-----------|--------|
| `engine_new` | `() -> Result[WasmEngine, Str]` | ensures result.is_ok() \|\| result.is_err() | ✅ |
| `engine_delete` | `(engine: WasmEngine)` | requires engine > 0 | ✅ |
| `store_new` | `(engine: WasmEngine) -> Result[WasmStore, Str]` | requires engine > 0 | ✅ |
| `store_delete` | `(store: WasmStore)` | requires store > 0 | ✅ |
| `module_new` | `(store: WasmStore, wasm_bytes: &Vec[UInt8]) -> Result[WasmModule, Str]` | requires store > 0, wasm_bytes.len() > 0 | ✅ |
| `module_delete` | `(module: WasmModule)` | requires module > 0 | ✅ |
| `instance_new` | `(store: WasmStore, module: WasmModule, imports: Int) -> Result[WasmInstance, Str]` | requires store > 0, module > 0, imports >= 0 | ✅ |
| `instance_delete` | `(instance: WasmInstance)` | requires instance > 0 | ✅ |
| `func_call` | `(func: Int, args: Int, results: Int) -> Result[Int, Str]` | requires func > 0, args >= 0, results >= 0 | ✅ |
| `functype_new` | `(param_types: Int, result_types: Int) -> Result[Int, Str]` | requires param_types >= 0, result_types >= 0 | ✅ |
| `valtype_new` | `(kind: Int) -> Result[Int, Str]` | requires kind >= 0, kind <= 3 | ✅ |
