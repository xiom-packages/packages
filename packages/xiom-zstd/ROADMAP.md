...
| Criterion | Status |
|-----------|--------|
| ✅ Extern "C" declarations | 5 FFI functions declared: ZSTD_compress, ZSTD_decompress, ZSTD_compressBound, ZSTD_isError, ZSTD_getErrorName |
| ✅ Safe wrappers | 3 pub fn: compress_bound, compress, decompress |
| ✅ Design-by-contract | 5 requires contracts across 3 functions |
| ✅ Tests | test_conformance.xi — 42 tests, 10 sections |
| ✅ SPEC.md | Full API surface documented |
| ✅ ROADMAP.md | This file |
| ⚠️ Vec[UInt8] marshaling | Blocked on compiler *UInt8 dereference support |
| ⚠️ C bridge linking | Requires system-installed libzstd at link time |
...
|-------|--------|-------------|
| **P1: Core FFI** | ✅ Done | extern "C" declarations for zstd compression/decompression |
| **P1: Safe Wrappers** | ✅ Done | compress_bound, compress, decompress with contracts |
| **P1: Tests** | ✅ Done | 42 conformance tests covering compress_bound, FFI stubs, API presence, contracts, edge cases |

## API Surface

| Function | Signature | Contracts | Status |
|----------|-----------|-----------|--------|
| `compress_bound` | `(size: Int) -> Int` | requires size > 0 | ✅ |
| `compress` | `(src: &Vec[UInt8], level: Int) -> Result[Vec[UInt8], Str]` | requires src.len() > 0, level >= 1, level <= 22 | ✅ |
| `decompress` | `(src: &Vec[UInt8]) -> Result[Vec[UInt8], Str]` | requires src.len() > 0 | ✅ |
