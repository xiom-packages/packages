...
2026-09-23 handoff; refreshed after wave 29). Check
...
- Folder inventory (2026-09-25, after wave 29): **285 implemented dirs (with
  `package.xi`) + 210 README-only placeholders + the umbrella
  `packages/package.xi`**. Since the 2026-09-23 handoff: 218 new packages were
  implemented (85 by converting placeholders, 133 brand-new dirs); the four
  ...
  `tzif`, `au`, `sbv`, `ktx`, `osrelease`, `duration`, `tcx`).
- `STATUS.json` totals: **218 stable, 4 ported, 63 incubating**;
  **222 green suites, 4,841 recorded tests**.
  - stable = the 218 greenfield packages (all `publish: false`, waiting on
  ...
  - incubating = 63 legacy packages (includes `xiom.durable`, the renamed
    core, not yet ported).
- `.github/publish-allowlist.txt`: **270 names** = 218 stable ready + 52
...
- Namespace audit **resolved**: `namespace-check` reports
  **285 packages, 383 modules, 0 conflicts** (2026-09-25, after wave 29).
...
- 489 commits landed since the previous handoff (`b2bdde1..`).

### Commit trail (recent milestones)

| Commit | What |
|---|---|
| this | SESSION.md refresh after wave 29 |
...
& .\scripts\status.ps1 -Action validate           # 285/0 expected
& .\scripts\allowlist-guard.ps1                   # 270 allowlisted, 0 failures
...
- **Scope deltas pending** (relay to the registry/ops):
  ...
  - wave 29 (10 names): `nii, hcl, dds, tzif, au, sbv, ktx, osrelease,
    ...
    scoped).
  - 119 names total pending enumeration before their canaries can run.
...
2. **Phase 1 -- small greenfield packages: DONE and expanded.** 218 packages
   built, conformance-tested, `stable`, allowlisted (waves 1-29). All pass
...
1. **Scope deltas** for wave 18 (v6, 151 -> 161) and waves 19-29 (the deltas
   after v6); 119 names total pending before those canaries.
...
5. **Wave 30+ queue**: 210 placeholders remain; strong small candidates are
   more format/protocol codecs (`miniseed`, `qoi`, `woff`, `resolv`, `passwd`,
   `ass`, and other names that pass namespace-check).
...
- **Compiler session:** accepted the three fn-value/ABI repros
  ...
  **0 sites** (`Vec<X]` / `Vec[X>` patterns; the 18 sites across 7 files are
  ...
  `hcl` #1) recovered by one retry each. Working patterns (not bugs):
